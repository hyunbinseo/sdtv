...
import { digits, email, length, object, pipe, string, ulid, uuid } from 'valibot';
...
export const load = (async ({ locals, url }) => {
	...
	const base = {
		...
		otpInputAttributes: {
			minlength: loginOtpLength,
			maxlength: loginOtpLength,
			placeholder: new Array(loginOtpLength)
				.fill(0)
				.map((_, index) => index + 1)
				.join('')
	...
	if (!url.searchParams.has('code')) return base;
	...
	const magicLinkLogins = await db
		.select({
	...
	return { ...base, magicLink: magicLinkLogins.at(0) || null };
...
export const actions = {
	send: async ({ fetch, getClientAddress, locals, request, url }) => {
		...
		const form = parseOrErrorPage(
			object({ contact: pipe(string(), email()) }),
			formDataToObject(formData, { get: ['contact'] })
		);
		...
		const existingUser = await db.query.userTable.findFirst({
			...
			where: and(
				eq(userTable.contact, form.contact), //
		...
		const user =
			...
			(
				await db
					.insert(userTable) //
					.values({ contact: form.contact })
					.returning(pickTableColumns(userTable, ['id']))
			)[0];

		if (!existingUser && form.contact === MASTER_CONTACT)
		...
		if (dev) {
			...
			const response = await sendEmail(
				{ To: form.contact, ...t.template(magicLink, login.otp) },
